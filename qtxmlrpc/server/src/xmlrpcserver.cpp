...
void XmlRpcServer::slotReceiverDestroed( QObject * o )
{
      ...
      foreach( const QString& m, objectMethods[o] )
...
void XmlRpcServer::slotRequestReceived( HttpServer * p, const QHttpRequestHeader & h, const QByteArray & body )
{
      ...
      QString methodName = e.firstChild().toText().data();
    ...
    if ( s->inherits( "QSslSocket" ) )
      ...
      const MethodData& md = callbacks[path + methodName];
      bool invoked;
      if( md.isMethodDeffered ) {
            invoked = QMetaObject::invokeMethod( md.receiver, md.method.constData(),
                                                 Qt::DirectConnection,
